...
export function PerformanceDashboard() {
  ...
  const { data: userPerf, isLoading: usersLoading } = useUserPerformance(selectedDept !== "all" ? selectedDept : undefined);
  const { data: deptPerf, isLoading: deptLoading } = useDepartmentPerformance();
  const { data: ticketPerf, isLoading: ticketsLoading } = useTicketPerformance(selectedDept !== "all" ? selectedDept : undefined);
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="departments" className="space-y-4">
        <TabsList className="bg-white/5">
          ...
          <TabsTrigger value="tickets"><TrendingUp className="h-4 w-4 mr-2" />Tickets</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="users" className="space-y-4">
          {usersLoading ? <Skeleton className="h-[500px]" /> : (
            <Card className="bg-white/5 border-white/10">
              <CardHeader>
                <CardTitle>User Response Performance</CardTitle>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {userPerf?.filter(u => u.total_responses > 0 || u.total_quotes_submitted > 0).map((u) => (
        ...
        <TabsContent value="tickets" className="space-y-4">
          {ticketsLoading ? <Skeleton className="h-[500px]" /> : (
            <Card className="bg-white/5 border-white/10">
              <CardHeader>
                <CardTitle>Ticket Response Details</CardTitle>
